            ...
            if (idx1 < 0 || idx2 < 0) {
                ...
                if (slot == -1) {
                    ...
                    if (target_idx != -1) {
                        ...
                        /* Informer les groupes à fusionner (via ORDRE_MGR) afin qu'ils
                         * avertissent leurs clients de rejoindre le nouveau groupe.
                         * Nous enverrons un message MGR MIGRATE <new_name> <new_port>
                         */
                        ISYMessage migr_msg;
                        memset(&migr_msg, 0, sizeof(migr_msg));
                        strcpy(migr_msg.ordre, ORDRE_MGR);
                        ...
                        /* Envoyer au port des 2 groupes pour qu'ils notifient leurs clients */
                        ...
                        /* Fermer les groupes historiques */
                        groupes[idx1].actif = 0;
                        groupes[idx2].actif = 0;
                        /* Si on a PID, on peut SIGTERM les processus GroupeISY */
                        if (groupes[idx1].pid > 0) {
                            pid_t pid1 = groupes[idx1].pid;
                            kill(pid1, SIGTERM);
                            /* Non-blocking wait with timeout */
                            int gaveup = 0;
                            for (int t=0; t<30; ++t) { /* wait up to 3s */
                                pid_t r = waitpid(pid1, NULL, WNOHANG);
                                if (r == pid1) { break; }
                                msleep_ms(100); /* 100ms */
                                if (t == 29) { gaveup = 1; }
                            }
                            if (gaveup) { /* force kill */
                                kill(pid1, SIGKILL);
                                waitpid(pid1, NULL, 0);
                            }
                            groupes[idx1].pid = 0;
                        ...
                        if (groupes[idx2].pid > 0) {
                            ...
                            int gaveup2 = 0;
                            for (int t=0; t<30; ++t) {
                                pid_t r = waitpid(pid2, NULL, WNOHANG);
                                if (r == pid2) { break; }
                                msleep_ms(100);
                                if (t == 29) { gaveup2 = 1; }
                            }
